...
class TrainerUtils:
    def __init__(
        ...
        self.checkpoint_path: str = os.path.join(
            model_checkpoint_path, f"model_{experiment_tag}"
        )
    ...
    def do_checkpoint(
        self, epoch: int, score: float, model, accuracy_df: pd.DataFrame = None
    ):
    ...
    def forward_step(
        self, batch, model
    ) -> Tuple[torch.Tensor, float, list, list, torch.Tensor]:
    ...
    def training_step(
        ...
        loss, accuracy, labels_predicted, labels, logits = self.forward_step(
            batch, model
        )
    ...
    def test_step(
        ...
        batch[2] = batch[2].to(self.device)
        with torch.no_grad():
            loss, accuracy, labels_predicted, targets, logits = self.forward_step(
                batch, model
            )
        ...
        for epoch in range(epoch_num):
            ...
            for step_number in range(epoch_len):
                ...
                if self.tensorboard is not None and step_number % validate_every == 0:
                    ...
                    self.log_metrics_accuracy(
                        train_accuracy, "train", self._global_steps_num
                    )
                    self.log_metrics_accuracy(
                        val_accuracy, "val", self._global_steps_num
                    )

            # прогоняем модель на тесте/вале и сохраняем если она лучше по accuracy
            predictions, _, targets, _ = self.predict(
                model, val_dataloader, verbose=verbose
            )
